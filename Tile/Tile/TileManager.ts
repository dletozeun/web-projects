class TileManager
{
    ...
    constructor()
    {
        ...
        //EventUtil.addHandler( document, "mousemove", ( event: MouseEvent ) => this.onMouseMoveHandler( event ) );
    }

    createTile( position: Vector2, size: Vector2, color: string, text: string ): Tile
    {
    ...
    createTileGrid( tileSize: number, tileCount: Vector2, offset: Vector2 )
    {
        this.offset = offset;
    ...
    update()
    {
        ...
        var pos = new Vector2( Math.floor( 0.5 * this.gridSize.x + 0.4 * this.gridSize.x * ( 1 + Math.sin( 1 * time ) ) * 0.5 * ( Math.cos( 0.20 * time ) + Math.cos( 0.10 * time + 1.5 ) ) ),
                               Math.floor( 0.5 * this.gridSize.y + 0.4 * this.gridSize.y * ( 1 + Math.sin( 1 * time ) ) * 0.5 * ( Math.sin( 0.15 * time ) + Math.sin( 0.007 * time + 0.7 ) ) ) );
        ...
        for ( var i = 0; i < this.tiles.length; ++i )
        {
            ...
            // change color for every tile except the 129th one which is the linked in logo
